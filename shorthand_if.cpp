			...
			if (line.at(line.size() - 2) != ":") {
				...
				/* Initial Time is time from CDFG. Needed to compute slack for List_R scheduling */
					...
					if (is_in1 == true && is_in2 == true && is_in3 == true) {
						...
						vertices.at(getIndex(var0)).setInitialTime(max(vertices.at(getIndex(var1)).getInitialTime() + vertices.at(getIndex(var1)).getDelay(), vertices.at(getIndex(var2)).getInitialTime() + vertices.at(getIndex(var2)).getDelay(), vertices.at(getIndex(var3)).getInitialTime() + vertices.at(getIndex(var3)).getDelay()));
